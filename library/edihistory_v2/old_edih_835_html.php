 ...
 * edih_835_html.php
 * 
 * Copyright 2016 Kevin McCormick
 * 
 ...
 */
 function edih_pmt_code_text ( $code_type, $code_str ) {
 ...
 */
 function edih_payment_vals ( $ar_st_slice, $elem_delim, $comp_delim ) { 
	 //
	 $ar_clm = array();
	 $svc_ct = -1;
	 $clp_ct = -1;
	 ...
	 foreach ( $ar_st_slice as $segtext ) {
		 ...
		 if ($seg[0] == "BPR" ) {
			 ...
			 $ar_clm['BPR']['date_pmt'] = edih_format_date($seg[16]);
		 ...
		 if ( $seg[0] == "TS3" ) {
			 ...
			 // see also edih_835_EOB_heading_html for constructing html table header
			 // 
		 ...
		 if ($seg[0] == "PER" ) { 
			 if ($loop == "1000A") {
				 if ($seg[1] == "CX") {
					 ...
					 if ($seg[3] == "TE") { 
						 ...
						 $ar_clm['BPR']['payer_contact'] = edih_format_telephone($seg[4]);
					 } else {
				 ...
				 if ($seg[1] == "BL") {
					 ...
					 for ($i=3; $i<count($seg); $i=$i+2) {
						 if ($seg[$i] == "TE" || $seg[$i] == "FX") { 
							 $pr_tech .= $seg[$i].' '.edih_format_telephone($seg[$i+1]).' ';
						 } else {
			 ...
			 if ($loop == "2100") {
				 ...
				 if ($seg[3] == "TE") { 
					 //$ar_clm['LX'][$lx_ct]['CLM'][$clp_ct]['payer_contact'] = substr($seg[4],0,3) . "-" . substr($seg[4],3,3) . "-" . substr($seg[4],6);
					 $ar_clm['LX'][$lx_ct]['CLM'][$clp_ct]['payer_contact'] = edih_format_telephone($seg[4]);
				 } else {
		 ...
		 if ($seg[0] == "AMT" ) { 
			 ...
			 $amt_a .= is_numeric($seg[2]) ? edih_format_money(trim($seg[2])) : trim($seg[2]); 
			 $amt_a .= ' ';
			 //
			 $amt_c .= $seg[1] .' ';
			 //
 ...
 * @return string
 */
function edih_835_pmt_summary($filename, $clm01) {
	...
	//
	if ($obj835) {
		//$ar_segs = $obj835->edih_segments();
		$delims = $obj835->edih_delimiters();
		$ds = $delims['s'];
		$de = $delims['e'];
		$ar_trans = $obj835->edih_x12_transaction($clm01);
		$sp = '';
	} else {
		// log entry from csv_check_x12_obj()
		return "Invalid file name $filename <br />" . PHP_EOL;
	}
	//
	if ( count($ar_trans) ) {
		$trns_ct = count($ar_trans);
		$idx = 0;
	} else {
		csv_edihist_log("ibr_era_claim_popup: invalid claim id");
		return "Did not locate payment information for $clm01 in " . $obj835->edih_filename() . "<br />" . PHP_EOL;
	}
	// transaction[i][j]  [i] to $ar_clp
	foreach($trans as $ar_clp) {
		$idx++;
		$svc_adj_codes = '';
		//
		$ar_eob = edih_payment_vals($ar_clp, $de, $ds);
		//
		...
		$sp .= "<table class='summaryRA' cols=4><caption>ERA Summary $clm01 ($idx of $trns_ct)</caption>".PHP_EOL;
		...
		foreach ($ar_eob['LX'][0]['CLM'] as $clm) {
		 ...
		 //
		if ($svc_adj_codes) { 
			$svc_code_text = edih_pmt_code_text ("CLMADJ", $svc_adj_codes );
...
/**
 * Generate the table heading for the tables created in edih_835_pmt_html ()
 * 
 ...
 */
function edih_835_EOB_heading_html ( $section = "CLM", $fmt_ar = NULL) {
 ...
 * 
 * This function is called from function edih_835_html_page() in this script. It calls 
 * function edih_835_EOB_heading_html() to generate table headings <thead></thead>
 ...
 */
function edih_835_pmt_html ($ar_clpvals, $fname = "835 Remittance Advice", $items = "ALL" ) {
	...
	if (array_key_exists("BPR", $ar_clpvals ) ) { 
		...
		$clp_html .= edih_835_EOB_heading_html ("BPR");
	...
	if (array_key_exists("PLB", $ar_clpvals ) ) {
		$clp_html .= edih_835_EOB_heading_html ("PLB");
		...
		foreach ($ar_clpvals["PLB"] as $plb) {
			...
			foreach ( $plb['itm'] as $itm) {
				$clp_html .= "<tr align=\"left\" class=\"plb\">".PHP_EOL;   
				$clp_html .= "<tr>".PHP_EOL."<td>{$itm['amount']}</td><td>{$itm['identifier']}</td><td>{$itm['text']}</td>".PHP_EOL; 
				$clp_html .= "</tr>".PHP_EOL;
			  //['provider']['per_date']["identifier_$i"]["amount_$i"]["text_$i"]
			}
			$clp_html .= "</tbody>".PHP_EOL."</table>".PHP_EOL;
	...
	if (array_key_exists("LX", $ar_clpvals) ) { 
		...
		foreach($ar_clpvals["LX"] as $ar_lx) {
			...
			if (array_key_exists("TS3", $ar_lx) ) {
				...
				//
				$lx_ts3 = '';
				$lx_ts3 .= edih_835_EOB_heading_html ("TS3", $ar_lx['TS3']);
				...
				$lx_ts3 .= "</tr>".PHP_EOL."</tbody>".PHP_EOL."</table>".PHP_EOL; 
			} // end if (array_key_exists("TS3"
			
			if (array_key_exists("CLM", $ar_lx) ) {
				//
				$lx_head .= edih_835_EOB_heading_html ("CLM");	 
				...
				foreach ($ar_lx['CLM'] as $val) {
					...
					if (array_key_exists("corr_last", $val)) {
						...
						$lx_body .= PHP_EOL."<td colspan=2><em>Corrected: </em> $corr_str</td>";
					} else {
						$lx_body .= PHP_EOL."<td colspan=2>&nbsp;</td>";
					}					   						     
					...
					if (array_key_exists("moa", $val)) {
						...
						$lx_body .= PHP_EOL."<td align=center><em>Rem Code</em> {$val['moa']}</td>";
					} else {
					$lx_body .= "<td>&nbsp;</td>";
					} 
					$lx_body .= PHP_EOL."</tr>".PHP_EOL; 
					...
					if (array_key_exists("moa_amt", $val) && strlen($val['moa_amt']) > 0) {
						...
						$lx_body .= "<tr>".PHP_EOL."<td colspan=6>{$val['moa_amt']}</td>".PHP_EOL."</tr>".PHP_EOL;
					}
					//
					...
					if ($clm_amt_codes) {
						...
						foreach ( $ar_amt_rem as $cd ) { 
							$lx_foot .= "<tr class=\"code\">".PHP_EOL."<td align=\"center\">{$cd[0]}</td>".PHP_EOL;
							$lx_foot .= "<td colspan=5>{$cd[1]}</td></tr>".PHP_EOL;
					}
				...
				if ($lq_rem_codes) {
					...
					foreach ( $ar_lq_rem as $cd ) { 
						$lx_foot .= "<tr class=\"code\">".PHP_EOL."<td align=\"center\">{$cd[0]}</td>".PHP_EOL."<td colspan=5>{$cd[1]}</td>".PHP_EOL."</tr>".PHP_EOL;
					}
					// reset
					$lq_rem_codes = "";
					//$lx_foot .= "</tbody>";
				...
				if ($clm_adj_type) { 
					...
					foreach ( $ar_clm_type as $tp ) { 
						$lx_foot .= "<tr class=\"code\">".PHP_EOL."<td align=\"center\">{$tp[0]}</td>".PHP_EOL."<td colspan=5>{$tp[1]}</td>".PHP_EOL."</tr>".PHP_EOL;
					}
					// reset 
					$clm_adj_type = "";
					//$lx_foot .= "</tbody>";	
				...
				if ($svc_adj_codes) { 
					...
					foreach ( $ar_svc_codes as $cd ) { 
						$lx_foot .= "<tr class=\"code\">".PHP_EOL."<td align=\"center\">{$cd[0]}</td>".PHP_EOL."<td colspan=5>{$cd[1]}</td> 
						</tr>";
					}
					// reset 
					$svc_adj_codes = "";
 ...
 * @param string $trn_trace     optional trace number
 ...
 */		
function edih_835_html_page ( $file_path, $trn_trace='', $pid_enctr='', $searchtype='ALL', $fname='835 Remittance Advice') {
	// 
	...
	$ar_era_segments = "";
	$srchstr = "";
	$srchtxt = strtolower($searchtype);
	//
	$obj835 = csv_check_x12_obj($file_path, 'f835');
	//
	if ($obj835 instanceof edih_x12_file) {
		$fn = $obj835->edih_filename();
		$de = $obj835->edih_delimiters()['e'];
		$ds = $obj835->edih_delimiters()['s'];
	} else {
		csv_edihist_log("edih_835_html_page: failed verification $file_path");
		return "Did not find or failed verification for $file_path <br />" . PHP_EOL;
	}
	//
	if ($srchtxt && $srchtxt != 'all') {
		$srchstr = (strpos($srchtxt, 'enc') !== false)  ? "Enctr: $pid_enctr" : '';
		$srchstr = (strpos($srchtxt, 'pid') !== false)  ? "PtID: $pid_enctr" : $srchstr;
		$srchstr = (strpos($srchtxt, 'tra') !== false)  ? 'Trace' : $srchstr;
	} else {
		$srchstr = 'all';
	}
	//	
	// select the kind of information we want claim, ST or file
	if ( $pid_enctr && ($srchstr == "Enctr: $pid_enctr" || $srchstr == "PtID: $pid_enctr") )  {
		// we are looking for a specific claim
		$ar_clp_slice = $obj835->edih_x12_transaction($pid_enctr);
		//
		$ar_clp = array();
		//
		if ( count($ar_clp_slice) ) {
			foreach($ar_clp_slice as $clp) {
			...
			csv_edihist_log("edih_835_html_page: Claim $pid_enctr not found in $fn");
		}
		...
		if (!empty($ar_clp) ) {
			...
			$ar_eob_vals = edih_payment_vals($ar_clp, $elem_d, $comp_d);
			
			$ar_eob_str .= edih_835_pmt_html($ar_eob_vals, $fn, $srchstr );
			// 
		} else {
			$ar_eob_str = "Claim $pid_enctr not found in $fn";
			csv_edihist_log("edih_835_html_page: Claim $srchstr not found in $fn");
		}
	} else {
		...
		$ar_st_slices = $obj835->edih_envelopes()['ST'];
		$ar_segs = $obj835->edih_segments();
		//
		if (!$trn_trace) { 
			...
			foreach ($ar_st_slices as $st) {
				//
				$ar_clp = array_slice($ar_segs, $st['start'], $st['count']);
				$ar_eob_vals = ibr_era_claim_vals($ar_clp, $de, $ds);
				$ar_eob_str .= ibr_era_claim_html ($ar_eob_vals, $fname, "ALL Items" );
				//
				csv_edihist_log("edih_835_html_page: era HTML output for $file_path");
			}
		} else {
			// a specific transaction trace is requested
			foreach ($ar_st_slices as $st) {
				if ($st['trace'] == $trn_trace) { 	 
					//
					$is_found = TRUE;
					$ar_clp = array_slice($ar_segs, $st['start'], $st['count']);
					$ar_eob_vals = edih_payment_vals($ar_clp, $de, $ds);
					$ar_eob_str .= edih_835_pmt_html($ar_eob_vals, $fn, "Trace: $trn_trace" );
					//
					csv_edihist_log("edih_835_html_page: HTML output for $trn_trace in $fn");
					//
					break;
				} else {
			...
			if (!$is_found) {
				...
				csv_edihist_log("edih_835_html_page: trace $trn_trace not found in era file $fn TMP: $file_path");
				$ar_eob_str = "<p>Trace $trn_trace not found in $fn TMP: $file_path.</p>";
			}
		}
	}
	if (!$ar_eob_str) {
		$ar_eob_str = "<p>edih_835_html_page: failed to return anything for Trace $trn_trace Claim $claim_id in $file_path</p>";
		csv_edihist_log("edih_835_html_page: failed to return anything for Trace $trn_trace Claim $claim_id in $file_path");
	}
	return $ar_eob_str;
}		 	 
