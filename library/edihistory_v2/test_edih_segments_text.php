...
function edih_837_text($segments, $delimiter, $err_seg = '') {
	...
	foreach ($segments as $key=>$seg) {
		...
		if ( count($err_ar) ) {
			if ( isset($er['stn']) ) {
				if ($er['stn'] == $stn) {
					foreach($err_ar as $er) {
						if ( int_val($er['segpos']) == $stsegct) {
							$bterr = 'bterr';
						}
					}
				}
		...
		if ( strncmp('SVD'.$de, $seg, 4) === 0 ) {
			...
			$str_html .= "<tr></tr><td class='btloop' title='$title'>$loopid</td><td class='btnum'>$key</td><td class='$bterr'>$seg</td></tr>" .PHP_EOL;
			$title = '';
			continue;			
		}
		//
		...
		$str_html .= "<tr><td class='btloop' title='$title'> -- </td><td class='btnum'>$key</td><td class='$bterr'>$seg</td></tr>" .PHP_EOL;	
		//
	}
	//
	return $str_html;	
...
function edih_display_text($filepath, $filetype='', $claimid='', $trace=false, $err_info='') {
	...
	$str_html .= "<th class='segtxt'>Segment (<em>File:</em> $fn) $hlnk</th></tr>".PHP_EOL;
