 ...
 * 
 * 
 * codes from http://www.wpc-edi.com/reference/
 * 
 */
...
// x12 835 (ERA) Claim Adjustment Reason Codes
/**
	...
	public function get_CODE_CLAIM_ADJUSTMENT( $adj_code ) {
		...
		if ( is_array( $this->CODE_CLAIM_ADJUSTMENT )){
			if (array_key_exists($adj_code, $this->CODE_CLAIM_ADJUSTMENT)){
				...
				'267' => 'Claim/service spans multiple months. Rebill as separate claim/service. This change effective 9/1/2015: Claim/service spans multiple months. At least one Remark Code must be provided (may be comprised of either the NCPDP Reject Reason Code, or Remittance Advice Remark Code that is not an ALERT.)',
 ...
 * Remittance Advice Remark Codes • ASC X12 External Code Source 411 LAST UPDATED 11/1/2015
 * 
 * @param $rem_code
 * @return array
 */
	public function get_CODE_RA_REMARK( $rem_code ) {
		...
		if ( is_array( $this->CODE_RA_REMARK )) {
			...
			$this->CODE_RA_REMARK = array(
				...
				'N755' => 'Missing/incomplete/invalid ICD Indicator on the 1500 Claim Form.',
...
//In Version 004010A1, the following PLB codes are available for use: 50, 51, 72,
//90, AM, AP, B2, B3, BD, BN, C5, CR, CS, CT, CV, CW, DM, E3, FB, FC, GO, IP,
//IR, IS, J1, L3, L6, LE, LS, OA, OB, PI, PL, RA, RE, SL, TL, WO, WU, AND ZZ. In
//version 005010, two new codes – AH and HM – have been added, and code ZZ
//has been deleted. The other change in Version 005010 is the way situational field
//PLB03-2 for reference identification is used.

/**
 * PLB reason code from PLB segment
 * "unknown" for codes with no definition found
 * 
	...
	public function get_CODE_PLB_REASON( $clm_stat_code ) {
		...
		if ( is_array( $this->CODE_PLB_REASON )) {
			if (array_key_exists($clm_stat_code, $this->CODE_PLB_REASON)){
				...
				return $this->CODE_PLB_REASON["ZZ9X"];;
			}
		} else {
			$this->CODE_PLB_REASON = array (	
			"50" => "Late Charge",
			"51" => "Unknown",
			"72" => "Authorized return Refunds - Manual Invoices",
			...
			"WU" => "Federally mandated recoupment/bonus payment",
			"ZZ" => "Unknown",
			"ZZ9X" => "Unknown Code"
			);
			if (array_key_exists($clm_stat_code, $this->CODE_PLB_REASON)){
 ...
 *   code, meaning, loop, category description
